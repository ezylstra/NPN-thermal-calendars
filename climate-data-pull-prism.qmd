...
if (need_yest_only == FALSE & up_to_date == FALSE) {
  ...
  if (need_yest_only) {
    ...
    # Date without dashes
    yest_nodash <- str_remove_all(yesterday, "-")
    
    # Create folder/zip names
    min_folder <- paste0("prism_tmin_us_25m_", yest_nodash)
    min_zip <- paste0(prism_folder, min_folder, ".zip")
    max_folder <- paste0("prism_tmax_us_25m_", yest_nodash)
    max_zip <- paste0(prism_folder, max_folder, ".zip")
    ...
    req_min <- req %>%
      req_url_path_append("tmin", yest_nodash)
    invisible(req_perform(req_min, path = min_zip))
    
    req_max <- req %>%
      req_url_path_append("tmax", yest_nodash)
    invisible(req_perform(req_max, path = max_zip))
