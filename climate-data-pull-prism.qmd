...
calc_gdd_be <- function(tmin = NULL, tmax = NULL, base = 32) {
  ...
  .mapply(function(tmin, tmax) { # For each day...
    ...
    # If tmin is just slightly greater than tmax, print warning but don't stop execution
    if (tmin - tmax > 0.5) {
      stop("tmin > tmax!")
    } 
    if (tmin - tmax <= 0.5 & tmin - tmax > 0) {
      warning("tmin - tmax between 0 and 0.5 for one or more cells")
      tmin <- tmax
    }
    # Step 2
    if (tmax < base) {
